pipeline {
    ...
    stages {
        stage('Download kernel') {
            steps {
                    sh '''
                        bash /opt/common/scripts/1_fetch_kernel.sh main
                    '''
                }
        }
        ...
        stage('Root the Android Boot Image') {
            steps {
                sh '''
                    ...
                    cp /out/root-boot.img "${WORKSPACE}/astro_root_boot_a_keyboard-patch.img"
                    '''
            }
        }
        ...
        stage('Copy boot image to master node') {
            steps {
                sh 'cp /out/root-boot.img "${WORKSPACE}/astro_root_boot_a_keyboard-patch.img"'
            }
        }

        stage('Publish boot image on S3') {
            steps {
               archiveArtifacts artifacts: 'astro_root_boot_a_keyboard-patch.img', onlyIfSuccessful: true
            }
        }
  }
}
